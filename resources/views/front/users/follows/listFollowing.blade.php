                                    ...
                                    <div class="user-long-sm-item bg-light border pl-15 pt-15 pr-15 pb-5 clearfix">
                                        <div class="image">
                                            {{ Html::image(config('asset.image_path.user_ava') .
                                                $userFollower->image, $userFollower->name) }}
                                        </div>
                                        <div class="content">
                                            <div class="block">
                                                @if(check_freelancer($userFollower))
                                                    <span class="label label-info">{{ $userFollower->role }}</span>
                                                @else
                                                    <span class="label label-warning">{{ $userFollower->role }}</span>
                                                @endif
                                            </div>
                                            <h4>{{ $userFollower->name }}</h4>
                                            <ul class="user-meta">
                                                <li><i class="fa fa-map-marker"></i> {{ $userFollower->address }}</li>
                                            </ul>
                                            <div class="row gap-10">
                                                <div class="col-xs-12 col-sm-9">
                                                    <ul class="user-meta">
                                                        @if(check_freelancer($userFollower))
                                                            <li>
                                                                {{ $userFollower->event_plans_count }}
                                                                {{ trans('list_follow.eventPlans') }}
                                                            </li>
                                                            <li>
                                                                {{ $userFollower->reviews_count }}
                                                        ...
                                                        @else
                                                            <li>
                                                                {{ $userFollower->request_events_count }}
                                                                {{ trans('list_follow.requestEvents') }}
                                                            </li>
                                                            <li>
                                                                {{ $userFollower->event_forks_count }}
                                                                {{ trans('list_follow.eventForks') }}
                                                            </li>
                                                            <li>
                                                                {{ $userFollower->comments_count }}
                                                ...
                                                <div class="col-xs-12 col-sm-3">
                                                    <a href="{{ route('userProfile', $userFollower->id) }}"
